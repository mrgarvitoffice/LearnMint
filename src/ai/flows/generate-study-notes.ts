...
const generateStudyNotesPrompt = aiForNotes.definePrompt({
  ...
  prompt: `You are a highly intelligent academic AI tutor. Your task is to generate top-quality, exam-focused study notes on any given topic. Your output MUST be a single, well-formatted Markdown string.
...
Cost estimation is the process of predicting the effort, duration, and cost required to develop a software project. It is used for budgeting, scheduling, and risk management.
...
2.  **COCOMO (Constructive Cost Model)**
    - A model that estimates project cost based on Lines of Code (LOC)
