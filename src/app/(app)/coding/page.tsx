...
type CodingAssistant = 'coder' | 'jarvis' | 'alya';
...
export default function CodingPage() {
  ...
  const [activeSideTab, setActiveSideTab] = useState('explorer'); // Changed default tab
  ...
  const [explorerHeight, setExplorerHeight] = useState(40); // Initial height percentage for explorer
  ...
  useEffect(() => {
    const initialMessages: Record<CodingAssistant, string> = {
        ...
        alya: "Alya here! Ready to add a little sparkle to your code. What are we building today?"
    };
  ...
  useEffect(() => {
    ...
    window.addEventListener('mousemove', handleExplorerResize);
    ...
    return () => {
      ...
      window.removeEventListener('mousemove', handleExplorerResize);
  ...
  }, [handleSideResize, handleStopSideResize, handleVerticalResize, handleStopVerticalResize, handleExplorerResize, handleStopExplorerResize]);
  ...
  const handleSendMessage = useCallback(async (messageText: string) => {
    ...
    const typingMessages: Record<CodingAssistant, string> = { coder: 'Coder AI is building...', jarvis: 'J.A.R.V.I.S. is processing...', alya: 'Alya is thinking...' };
        ...
        setFiles(prevFiles => {
          ...
          response.fileOperations?.forEach((op: FileOperation) => {
            ...
            if (op.operation === 'create') {
              ...
              } else { // It's an update in disguise
              ...
              else newFiles.push({ name: op.fileName, content: op.content || '', language: lang }); // Create if not found
  ...
  const ShortcutsDialogContent = () => (
    <DialogContent className="sm:max-w-xl bg-[#252526] border-gray-700 text-gray-300">
      <DialogHeader>
  ...
  const AiSwitcher = ({ mobile = false }: { mobile?: boolean }) => (
    <div className="flex items-center gap-1">
      ...
      <InteractiveCharacterElement
        ...
        className={cn("ring-2 ring-transparent", mobile ? "h-6 w-6" : "h-6 w-6", activeAi === 'alya' && 'ring-pink-500')}
        containerClassName="cursor-pointer"
        onClick={() => setActiveAi('alya')}
      />
    </div>
  );
        ...
        <div className="flex h-full w-full relative z-10">
            <div className="w-12 bg-[#333333] flex flex-col items-center py-4 space-y-6 text-gray-400 border-r border-gray-900/50 z-20">
                ...
                <Dialog>
                    ...
                    <DialogContent className="max-w-[80vw] w-[80vw] h-[70vh] p-0 border-gray-700 bg-[#1e1e1e] flex flex-col">
                        <DialogHeader className="p-2 border-b border-gray-800 flex-row items-center justify-between">
                            <DialogTitle className="text-gray-200 flex items-center gap-2 text-base">
                                <TerminalIcon className="h-4 w-4" /> Terminal
                            </DialogTitle>
                ...
                <Dialog>
                    ...
                    <DialogContent className="sm:max-w-md bg-[#252526] border-gray-700 text-gray-300 max-h-[80vh] overflow-y-auto">
                        <DialogHeader>
                ...
                <Dialog>
                    ...
                    <DialogContent className="sm:max-w-xl bg-[#252526] border-gray-700 text-gray-300 max-h-[80vh] overflow-y-auto">
                        <DialogHeader>
                    ...
                    <div style={{ height: `${100 - topPaneHeight}%` }} className="flex flex-col min-h-0">
                        <Tabs value={activeBottomTab} onValueChange={setActiveBottomTab} className="w-full flex-1 flex flex-col">
                            ...
                            <Dialog>
                                ...
                                <DialogContent className="sm:max-w-xl bg-[#252526] border-gray-700 text-gray-300 max-h-[80vh] overflow-y-auto">
                                    <DialogHeader><DialogTitle>Project Management</DialogTitle></DialogHeader>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4">
                                        <Button onClick={handleNewProject} variant="outline"><FilePlus className="h-4 w-4 mr-2"/>New Project</Button>
                                        <Button onClick={handleDownloadProject} variant="outline"><FileDown className="h-4 w-4 mr-2"/>Download Project</Button>
                                    ...
                                    <div className="pt-4 mt-4 border-t border-gray-700">
                                        <h3 className="font-semibold text-lg mb-2">Project Slots</h3>
                                        <div className="space-y-2">
                                        {Array.from({ length: MAX_PROJECT_SLOTS }).map((_, i) => {
                                            const slot = projectSlots.find(s => s.id === i + 1);
                                            return (
                                                <div key={i} className="flex items-center justify-between p-2 rounded-md bg-gray-900/50">
                                                <div className="flex items-center gap-2">
                                                    <div className="font-mono text-xs bg-gray-700 px-2 py-0.5 rounded">Slot {i + 1}</div>
                                                    {slot ? (
                                                    <div>
                                                        <p className="font-semibold text-sm">{slot.name}</p>
                                                        <p className="text-xs text-gray-400">Saved: {new Date(slot.timestamp).toLocaleString()}</p>
                                                    </div>
                                                    ) : <p className="text-sm text-gray-500 italic">Empty Slot</p>}
                                                </div>
                                                <div className="flex gap-1">
                                                    <Button size="sm" variant="secondary" onClick={() => handleSaveToSlot(i+1)}><Save className="h-4 w-4 mr-1.5"/>Save</Button>
                                                    {slot && <Button size="sm" variant="outline" onClick={() => handleLoadFromSlot(i+1)}>Load</Button>}
                                                    {slot && <Button size="sm" variant="destructive" onClick={() => handleDeleteSlot(i+1)}><Trash2 className="h-4 w-4"/></Button>}
                                                </div>
                                                </div>
                                            )
                                        })}
