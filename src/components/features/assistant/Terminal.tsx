...
const Terminal = () => {
  ...
  const { mode } = useSettings(); // Use settings for mode
        ...
        >
          ...
          >
            {terminalContent.map((item) => (
              <div
                ...
                className={`w-full ${item.type === 'user'
                    ? 'text-blue-400'
                    : item.type === 'ai'
                    ? mode === 'jarvis' ? 'text-cyan-400' : 'text-pink-400'
                    : item.type === 'error'
                    ? 'text-red-400'
                    : 'text-yellow-400'
                  }`}
            ...
            <div className="flex items-center p-2 border-t border-gray-800 flex-shrink-0">
              <span className={`mr-2 flex-shrink-0 ${mode === 'jarvis' ? 'text-cyan-400' : 'text-pink-400'}`}>
                {mode === 'jarvis' ? 'J.A.R.V.I.S.' : 'Alya'} $
              </span>
              <input
                ref={inputRef}
                type="text"
