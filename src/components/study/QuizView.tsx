  ...
  const handleDownloadResultsPpt = () => {
    ...
    pptx.defineSlideMaster({
      ...
      objects: [
        ...
        { 'text': { text: 'Generated by LearnMint AI', options: { x: 0.5, y: 5.3, w: '90%', h: 0.2, align: 'left', fontFace: 'Arial', fontSize: 10, color: '94A3B8' } } },
        { 'image': { path: '/icons/icon-192x192.png', x: 9.2, y: 0.3, w: 0.5, h: 0.5, anim: { type: 'zoom', duration: 1, delay: 0.5 } } }
      ]
    });
    
    const titleSlide = pptx.addSlide({ masterName: 'HACKER_MASTER' });
    titleSlide.addText(`Quiz Review: ${topic}`, {
      x: 0.5, y: 2, w: '90%', h: 1,
    ...
    const totalPossibleScore = questions.length * 4;
    titleSlide.addText(`Final Score: ${score}/${totalPossibleScore}`, {
      x: 0, y: 3.2, w: '100%', h: 1,
    ...
    questions.forEach((q, index) => {
      ...
      if (q.type === 'multiple-choice' && q.options) {
        q.options.forEach(opt => {
          let color = 'E2E8F0'; // Default color for options
          let isUserChoice = (opt === userAnswer);
          let isTheCorrectAnswer = (opt === q.answer);
          if (isTheCorrectAnswer) color = '22C55E'; // Green for correct answer
          if (isUserChoice && !isCorrect) color = 'EF4444'; // Red for wrong user answer
      ...
      if(q.explanation) {
         slide.addText([{
          text: 'Explanation: ', options: { fontSize: 14, color: '94A3B8', bold: true }
         }, {
    ...
    pptx.writeFile({ fileName: `LearnMint_Quiz_Review_${topic}.pptx` });
  };
    ...
    return (
      <Card className="mt-0 shadow-lg flex-1 flex flex-col min-h-0">
        ...
        <CardFooter className="flex flex-col sm:flex-row justify-center gap-3 p-6 border-t">
          ...
          <Button variant="secondary" onClick={handleDownloadResultsPpt} size="lg"><MonitorPlay className="w-4 h-4 mr-2"/>Download PPT</Button>
        </CardFooter>
      </Card>
    );
  }
