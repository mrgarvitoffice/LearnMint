...
import type { YoutubeSearchInput, YoutubeSearchOutput, YoutubeVideoItem, GoogleBooksSearchInput, GoogleBooksSearchOutput, GoogleBookItem, GenerateAudioSummaryOutput as ClientGenerateAudioSummaryOutput, CombinedStudyMaterialsOutput } from './types';


/**
 * generateNotesAction (Combined Material Generation)
 *
 * This server action orchestrates the generation of study notes, a quiz,
 * and flashcards based on a single topic input. It can also accept user-provided notes
 * as a primary source.
 *
...
export async function generateNotesAction(input: GenerateStudyNotesInput): Promise<CombinedStudyMaterialsOutput> {
  ...
  let notesForGeneration = input.notes;
  
  // If notes are from a PDF, summarize them first using the reliable flow.
  if (input.notes) {
      try {
          console.log(`[Server Action - ${actionName}] PDF content detected. Summarizing before generating materials...`);
          const summaryResult = await generateAudioSummary({ text: input.notes });
          if (summaryResult.summary) {
              notesForGeneration = summaryResult.summary;
              console.log(`[Server Action - ${actionName}] PDF summarized successfully.`);
          } else {
              console.warn(`[Server Action - ${actionName}] PDF summarization returned empty. Using full text.`);
          }
      } catch (e) {
          console.error(`[Server Action Error - ${actionName}] Error summarizing PDF notes:`, e);
          // Fallback to using the full text if summarization fails
      }
  }
  ...
  const quizInput: GenerateQuizFromNotesInput = { notesContent: notesResult.notes, numQuestions: 30 }; // Changed from 15 to 30
...
export async function generateFlashcardsAction(input: GenerateFlashcardsInput): Promise<GenerateFlashcardsOutput> {
  ...
  try {
    ...
    if (error.message && (error.message.includes("GOOGLE_API_KEY") || error.message.includes("API key is invalid") || error.message.includes("API_KEY_INVALID"))) {
      clientErrorMessage = "Flashcard Generation: Failed due to an API key or project configuration issue. Please check server configuration.";
    } else if (error.message) {
